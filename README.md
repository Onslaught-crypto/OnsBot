...
| `structure.py` | swing highs/lows, strong vs weak (swept) classification, liquidity |
| `zones.py` | order blocks and fair value gaps, with fill/mitigation tracking |
| `tradeplan.py` | entry / SL / TP1-3 / fibs / R:R from structure + zones |
| `chart.py` | renders the candlestick chart image attached to each alert |
...
- Each alert includes a **candlestick chart image** (price with EMAs and
  Bollinger bands, a volume panel and an RSI panel). Turn it off with
  `SEND_CHARTS = False` in `config.py`.
- Each alert also carries a **trade plan** (`SHOW_TRADE_PLAN`): entry (CMP plus an
  optional limit at a fib/OB/FVG pullback), stop-loss beyond the nearest **strong
  swing** (one that swept prior liquidity), and **TP1-3** on unswept liquidity
  (weak swings, equal highs/lows, unfilled FVGs) with Fibonacci extensions as
  fallback — each target labeled with its **R:R**. Order blocks and fair value
  gaps show as context; set `ZONES_COUNT_AS_CONFLUENCE = True` to let a zone tap
  count toward the confluence gate. The chart draws the SL/entry/TP lines, the
  zones, and the strong swings, so you see the whole plan on the candles.
  These structure/zone concepts are approximations of discretionary ICT ideas —
  tune `SWING_WIDTH`, `OB_MOVE_ATR`, and the fib lists in `config.py` to taste.
